...
import type { EngramHooks, Memory, Message, MemoryCandidate } from '../types.js';
 ...
 * Run afterStore hook
 */
export async function runAfterStore(memory: Memory, hooks?: EngramHooks): Promise<void> {
  if (!hooks?.afterStore) {
    return;
  }
 ...
 */
export async function runBeforeRecall(query: string, hooks?: EngramHooks): Promise<string> {
