...
import { shouldPrune, sortByDecayedImportance, calculateExpiration } from './decay.js';
import {
  runBeforeStore,
  runAfterStore,
  runBeforeRecall,
...
const DEFAULT_CATEGORIES = ['fact', 'preference', 'skill', 'episode', 'context'];
...
export class Engram extends EngramEmitter {
  private config: Required<Omit<EngramConfig, 'llm' | 'embed' | 'hooks' | 'store'>> & {
    llm?: LLMFunction;
    embed?: EmbedFunction;
  ...
  constructor(config: EngramConfig = {}) {
    ...
    this.config = {
      ...
      importanceBoost: { ...DEFAULT_IMPORTANCE_BOOST, ...config.importanceBoost },
   ...
   */
  async remember(messages: Message[], options: RememberOptions = {}): Promise<RememberResult> {
    ...
    try {
      ...
      const processedMessages = await runBeforeExtract(messages, this.config.hooks);
      ...
      const filteredCandidates = await runAfterExtract(candidates, this.config.hooks);

      // Step 4: Load existing memories for scoring
      const existing = await this.storeAdapter.list({ namespace: this.config.namespace });

      // Step 5: Score each candidate
      for (const candidate of filteredCandidates) {
        const categoryBoost = this.config.importanceBoost[candidate.category] ?? 1.0;
        ...
        const memory: Memory = {
          ...
          lastAccessed: now,
          createdAt: now,
          embedding: this.config.embed ? await this.config.embed(candidate.content) : null,
          metadata: options.metadata ?? {},
          namespace: this.config.namespace,
          ttl: null,
          expiresAt: calculateExpiration({ createdAt: now, ttl: null } as Memory, this.config),
          version: 1,
          history: [],
          explanation: decision.explanation,
        };
  ...
  async store(content: string, options: StoreOptions = {}): Promise<Memory> {
    ...
    const memory: Memory = {
      ...
      expiresAt: calculateExpiration({ createdAt: now, ttl: options.ttl ?? null } as Memory, this.config),
      version: 1,
      history: [],
    };
    ...
    if (!approved) {
      throw new EngramError('Memory rejected by beforeStore hook', 'HOOK_REJECTED');
    }
  ...
  async forget(options: ForgetOptions = {}): Promise<ForgetResult> {
    ...
    const allMemories = await this.storeAdapter.list({ namespace: this.config.namespace });
    ...
    const oldest = remaining > 0
      ? Math.min(...allMemories.filter((m) => !prunedIds.includes(m.id)).map((m) => m.createdAt))
      : Date.now();
    ...
    for (const category of Object.keys(byCategory)) {
      ...
      for (let i = 0; i < memories.length; i++) {
        ...
        for (let j = i + 1; j < memories.length; j++) {
          ...
          // Compute similarity
          let similarity = 0;
          if (memories[i].embedding && memories[j].embedding && this.config.embed) {
            ...
            similarity = jaccardSimilarity(memories[i].content, memories[j].content);
          }
          ...
          if (similarity >= threshold) {
            ...
            if (this.config.trackHistory && !dryRun) {
              ...
              if (kept.history.length > this.config.maxHistoryPerMemory) {
                kept.history = kept.history.slice(-this.config.maxHistoryPerMemory);
              }
  ...
  async export(format: 'json' | 'md' | 'csv'): Promise<string> {
    const memories = await this.storeAdapter.list({ namespace: this.config.namespace });

    switch (format) {
      case 'json':
        return exportToJson(memories, this.config.namespace, this.config.categories);
      case 'md':
        return exportToMarkdown(memories, this.config.namespace, this.config.categories);
      case 'csv':
        return exportToCsv(memories);
      default:
        throw new EngramError(`Unknown export format: ${format}`, 'INVALID_FORMAT');
    }
  }
   ...
   */
  async import(data: string, format: 'json' | 'md', options: ImportOptions = {}): Promise<number> {
    ...
    if (format === 'json') {
      ...
      throw new EngramError(`Unknown import format: ${format}`, 'INVALID_FORMAT');
    }
    ...
    for (const mem of partial) {
      const memory: Memory = {
        ...
        ttl: mem.ttl ?? null,
        expiresAt: mem.expiresAt ?? calculateExpiration({ createdAt: now, ttl: null } as Memory, this.config),
