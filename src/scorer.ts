...
import type { Memory, MemoryCandidate, EmbedFunction, SurpriseResult } from './types.js';
 ...
 */
function isExactDuplicate(candidate: MemoryCandidate, existing: Memory[]): boolean {
  const normalizedContent = candidate.content.toLowerCase().trim();
  return existing.some((m) => m.content.toLowerCase().trim() === normalizedContent);
}
 ...
 */
function keywordNovelty(candidate: MemoryCandidate, existing: Memory[]): number {
  if (existing.length === 0) return 1.0;
 ...
 */
function categoryRarity(candidate: MemoryCandidate, existing: Memory[]): number {
  const countInCategory = existing.filter((m) => m.category === candidate.category).length;
...
function findMostSimilar(
  ...
  return mostSimilar ? { memory: mostSimilar, similarity: maxSimilarity } : null;
}
...
export async function computeSurprise(
  ...
  if (semantic !== null) {
    ...
    explanationText = explainSurprise(surprise, semantic, keyword, rarity, true);
  } else {
...
export async function scoreAndDecide(
  ...
  if (explain) {
    ...
    fullExplanation =
      result.explanation +
      '\n  ' +
      explainStoreDecision(result.surprise, importance, threshold, categoryBoost, stored, closestSim);
  }
